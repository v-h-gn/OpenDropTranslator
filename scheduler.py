...
def can_schedule(op: Op, tick: int, modules_busy: dict[str, int], available_modules: dict[str, int], current_droplets: int, max_droplets: int) -> bool:
    ...
    parent_modules_available = all(modules_busy.get(p.type, 0) < available_modules.get(p.type, 0) for p in dispense_parents)

    # Check if non-dispense parents have completed by this tick
    non_dispense_parents_scheduled = all(p.end_time <= tick for p in non_dispense_parents)

    # Check if op's required module is available
    op_module_available = modules_busy.get(op.type, 0) < available_modules.get(op.type, 0)

    # Check if scheduling this op would exceed max droplets
    new_droplets = current_droplets + len(dispense_parents)
    
    return parent_modules_available and non_dispense_parents_scheduled and op_module_available and (new_droplets <= max_droplets)
...
def list_scheduler(ops: list[Op], available_modules: dict[str, int], max_droplets: int | None = None) -> list[Op]:
    ...
    candidate_ops = [op for op in ops if op.parents and all([parent.is_input() for parent in op.parents])]
    ...
    while len(scheduled_ops) < len(ops):        
        ...
        scheduleable_ops = [op for op in candidate_ops if can_schedule(op, tick, modules_busy, available_modules, current_droplets, max_droplets)]
        ...
        for op in scheduleable_ops:
            ...
            for child in op.children:
                ...
                child_parents_scheduled = child.parents_scheduled(excluding=lambda p: p.is_input())
